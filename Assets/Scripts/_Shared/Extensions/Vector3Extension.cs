...
public static class Vector3Extension
{
    ...
    public static Vector3 RandomInBound2D(this Vector3 v, Bound2D bound)
    {
        float randX = Random.Range(bound.XLow, bound.XHigh);
        float randY = Random.Range(bound.YLow, bound.YHigh);
        v.x = randX;
        v.y = randY;
        return v;
    }
    ...
    {
        return new Vector3((float) Math.Round(vector3.x, digit, rounding),
            (float) Math.Round(vector3.y, digit, rounding),
    ...
    {
        return new Vector2((float) Math.Round(vector3.x, digit, rounding),
            (float) Math.Round(vector3.y, digit, rounding));
    }
}