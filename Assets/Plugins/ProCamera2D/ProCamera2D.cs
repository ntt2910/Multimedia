...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2D : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public float CameraMovementSmoothSpeed = 5f;
        ...
        {
            get { return new Vector2(_cameraTargetHorizontalPositionSmoothed, _cameraTargetVerticalPositionSmoothed); }
        ...
        public void Move(float deltaTime)
        {
            ...
            {
                CameraTargetPosition = VectorHV(Vector3H(ExclusiveTargetPosition.Value) - Vector3H(ParentPosition),
                    Vector3V(ExclusiveTargetPosition.Value) - Vector3V(ParentPosition));
            ...
            _cameraTargetHorizontalPositionSmoothed = Utils.SmoothApproach(_cameraTargetHorizontalPositionSmoothed,
                _previousCameraTargetHorizontalPositionSmoothed, Vector3H(CameraTargetPosition), 1f / HorizontalFollowSmoothness, DeltaTime);
            _previousCameraTargetHorizontalPositionSmoothed = _cameraTargetHorizontalPositionSmoothed;

            _cameraTargetVerticalPositionSmoothed = Utils.SmoothApproach(_cameraTargetVerticalPositionSmoothed, _previousCameraTargetVerticalPositionSmoothed,
                Vector3V(CameraTargetPosition), 1f / VerticalFollowSmoothness, DeltaTime);
            _previousCameraTargetVerticalPositionSmoothed = _cameraTargetVerticalPositionSmoothed;
            ...
            _transform.localPosition = Vector3.MoveTowards(_transform.localPosition,
                VectorHVD(Vector3H(newPos), Vector3V(newPos), Vector3D(_transform.localPosition)), Time.deltaTime * CameraMovementSmoothSpeed);

            //Debug.Log(_transform.localPosition);
        ...
        IEnumerator AdjustTargetInfluenceRoutine(CameraTarget cameraTarget, float influenceH, float influenceV, float duration,
        ...
        void OnDrawGizmos()
        {
            ...
            {
                if (IsRelativeOffset)
                    Utils.DrawArrowForGizmo(targetsMidPoint, VectorHV((OffsetX * cameraDimensions.x * .5f), (OffsetY * cameraDimensions.y * .5f)),
                        .04f * cameraDimensions.y);
                else
            ...
            if (Application.isPlaying)
            {
                ...
                var cameraTargetPosSmoothed = VectorHVD(_cameraTargetHorizontalPositionSmoothed + Vector3H(ParentPosition),
                    _cameraTargetVerticalPositionSmoothed + Vector3V(ParentPosition), cameraDepthOffset);
